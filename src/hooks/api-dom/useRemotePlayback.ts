...
/**
 * **`useRemotePlayback`**:
 * @see [RemotePlayback API](https://developer.mozilla.org/en-US/docs/Web/API/RemotePlayback).
 * @template T - The `HTMLMediaElement` subtype (`HTMLVideoElement`, `HTMLAudioElement`, etc.) that the ref will be attached to.
 * @see [📖 Documentation](https://react-tools.ndria.dev/hooks/api-dom/useRemotePlayback)
 * @param {UseRemotePlaybackProps} param - {@link UseRemotePlaybackProps}
 * @returns {UseRemotePlaybackResult} result {@link UseRemotePlaybackResult}
 */
