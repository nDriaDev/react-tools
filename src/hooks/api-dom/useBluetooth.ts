...
import { Bluetooth, BluetoothDevice, BluetoothDevicesOptions, BluetoothRemoteGATTServer, UseBluetoothResult, UseBluetoothState } from "../../models";
import { useSyncExternalStore } from "../state";

/**
 * **`useBluetooth`**: hook to connect and interact with bluetooth devices.
 * @see [Web Bluetooth API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Bluetooth_API).
 * @see [📖 Documentation](https://react-tools.ndria.dev/hooks/api-dom/useBLuetooth)
 * @returns {UseBluetoothResult} result {@link UseBluetoothResult}
 */
export const useBluetooth = (): UseBluetoothResult => {
	...
	const value: UseBluetoothState = useSyncExternalStore(
		useCallback(notif => {
