...
import { UseClipboardProps, UseClipboardResult } from "../../models";
...
/**
 * **`useClipboard`**: Hook to handle Clipboard.
 *
 * __*N.B.*__: The hook has the same compatibility issues as the Clipboard API for Firefox, i.e. it is currently impossible to read from the clipboard.
 * @see [Clipboard API](https://developer.mozilla.org/en-US/docs/Web/API/Clipboard_API).
 * @see [📖 Documentation](https://react-tools.ndria.dev/hooks/api-dom/useClipboard)
 * @param {UseClipboardProps} param - {@link UseClipboardProps}
 * @returns {UseClipboardResult} result {@link UseClipboardResult}
 */
...
function useClipboard({ useValue, dataType, target }: UseClipboardProps): UseClipboardResult {
	...
	const value = useSyncExternalStore(
		useCallback(notif => {
			...
			element?.addEventListener('paste', listener, {passive: true});
			return () => {
