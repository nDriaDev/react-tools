...
/**
 * **`useEffectAbortable`**: custom useEffect with a unified cancellation mechanism to ensure complete cleanup and to prevent the warning that appears on old React version _"Can't perform a React state update on an unmounted component"_.
 * @see [📖 Documentation](https://react-tools.ndria.dev/hooks/lifecycle/useEffectAbortable)
 ...
 * @returns {void} result
*/
export const useEffectAbortable = <T = unknown>(cb: UseEffectAbortableProps<T>["cb"], deps: UseEffectAbortableProps<T>["deps"]): void => {
	useEffect(() => {
