import { Dispatch, Reducer, ReducerAction, ReducerState, useCallback, useEffect, useRef } from "react";
import { useReducerHistory } from "."

/**
 * **`useReducerHistory`**: Custom useReducer with getter state function and that tracks and allows to use previous values.
 ...
 * @returns {[ReducerState<R>, Dispatch<ReducerAction<R>>, ()=>ReducerState<R>, history: {history: readonly ReducerState<R>[], presentPointer: number, trackUpdate: (enable:boolean) => void, canUndo: boolean, canRedo: boolean, undo: () => void, redo: () => void, go: (index: number) => void, clear: (value?: ReducerAction<R>) => void}]} array
 */
